...
if(isset($data["service_name"]) && isset($data["service_salt"])){
    ...
    if($sql_service_exists->fetchColumn() > 0){
        $sql1 = $pdo->prepare("SELECT * FROM active_storage WHERE service=?");
        $sql1->execute(array($service_name));
        $sql2 = $pdo->prepare("SELECT * FROM active_storage WHERE service=?");
        $sql2->execute(array($service_name));
        if($sql1->fetchColumn() > 0){
            ...
            echo '{"status":"success","message":"No storage found for this service."}';
        }
    }
    else{
